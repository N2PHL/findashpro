...
def _config_toml():
    ...
    if not p.exists():
        raise FileNotFoundError("thiếu .streamlit/config.toml — giao diện sẽ không ép nền trắng")
    cfg = tomllib.load(p.open("rb"))
    theme = cfg.get("theme", {})
    if theme.get("base") != "light":
        raise ValueError("theme.base phải là 'light' cho khớp biểu đồ plotly_white")
    if theme.get("backgroundColor", "").upper() not in {"#FFF", "#FFFFFF"}:
        raise ValueError("theme.backgroundColor phải là #FFFFFF (nền trắng)")
    from utils.config import PLOTLY_TEMPLATE
    if PLOTLY_TEMPLATE != "plotly_white":
        raise ValueError("PLOTLY_TEMPLATE phải là 'plotly_white' cho khớp nền sáng")
    return "ép giao diện sáng, nền trắng, khớp template biểu đồ"
