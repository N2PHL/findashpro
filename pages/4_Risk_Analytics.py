# pages/4_Risk_Analytics.py — yêu cầu [5] Monte Carlo + yêu cầu [4] CAPM & APT (một mã)
import numpy as np
...
from utils.config import (ACCENT, ACCENT_DOWN, ACCENT_UP, MARKET_INDEX, MUTED,
                          PRICE_UNIT, RISK_FREE_RATE, SIZE_INDEX)
...
def render_risk_analytics_page() -> None:
    ...
    with tab_mc:
        ...
        if not est["significant"]:
            note(f"<b>Vấn đề ước lượng tham số drift.</b> Với mẫu hiện tại, giả thuyết "
                 f"H₀: μ = 0 <b>không bị bác bỏ</b> (t = {est['tstat']:.2f}, khoảng tin cậy "
                 f"95% của μ năm hóa: [{est['ci_annual'][0]:+.1%}; {est['ci_annual'][1]:+.1%}] "
                 f"— khoảng này chứa 0). Nguyên nhân mang tính cấu trúc chứ không phải do "
                 f"mẫu xấu: sai số chuẩn của trung bình bằng σ/√n, nên độ chính xác của ước "
                 f"lượng drift chỉ cải thiện theo căn bậc hai của <i>độ dài lịch sử</i>, "
                 f"trong khi tăng tần suất quan sát hoàn toàn không giúp ích. "
                 f"<b>Merton (1980)</b> chỉ ra rằng để ước lượng μ với sai số hữu ích cần "
                 f"hàng chục năm dữ liệu, còn σ thì ngược lại — có thể ước lượng khá chính "
                 f"xác từ mẫu ngắn với tần suất cao. Với {est['n_obs']:,} phiên hiện có, sai "
                 f"số chuẩn (±{est['se_annual']:.2%}) lớn hơn chính giá trị ước lượng. Do đó "
                 f"khi mục tiêu là <b>đo lường rủi ro</b>, chuẩn mực là đặt drift ở mức "
                 f"<b>trung tính rủi ro</b> (μ = r_f), tức chuyển sang độ đo martingale "
                 f"tương đương, thay vì áp đặt một xu hướng mà dữ liệu không đủ sức khẳng "
                 f"định. Chọn drift lịch sử chỉ phù hợp khi câu hỏi nghiên cứu là dự báo "
                 f"kỳ vọng, và khi đó kết quả phải được báo cáo kèm khoảng tin cậy trên.")
        ...
        if sim is not None:
            ...
            note(f"<b>Giá trị chịu rủi ro (VaR) và VaR có điều kiện (CVaR).</b> VaR ở mức "
                 f"tin cậy {conf:.0%} là phân vị thứ {100*(1-conf):.0f} của phân phối lãi/lỗ: "
                 f"trong {horizon} phiên tới, xác suất khoản lỗ vượt quá "
                 f"<b>{m['var_amount']:,.2f} {PRICE_UNIT}</b> ({m['var_pct']:.2%} giá trị "
                 f"danh mục) là {100*(1-conf):.0f}%. VaR là một <b>đại lượng lỗ</b>, không "
                 f"phải một mức giá, và nó <i>không</i> cho biết khoản lỗ lớn đến đâu khi "
                 f"ngưỡng bị vượt. CVaR (còn gọi là Expected Shortfall) lấp khoảng trống đó "
                 f"bằng kỳ vọng có điều kiện E[L | L &gt; VaR] — mức lỗ trung bình trong "
                 f"{100*(1-conf):.0f}% kịch bản xấu nhất — nên theo định nghĩa luôn lớn hơn "
                 f"hoặc bằng VaR. <b>Artzner và cộng sự (1999)</b> chứng minh VaR không phải "
                 f"thước đo rủi ro nhất quán vì vi phạm tính dưới cộng tính: VaR của danh mục "
                 f"gộp có thể lớn hơn tổng VaR các thành phần, tức mô hình có thể ngầm phạt "
                 f"việc đa dạng hóa. CVaR thỏa mãn tiên đề này và vì vậy được Basel III ưu "
                 f"tiên sử dụng.")
            ...
            with cB:
                ...
                fig = go.Figure(go.Histogram(x=end, nbinsx=60, marker_color=ACCENT,
                                             opacity=0.8))
                fig.add_vline(x=m["s0"], line_color=MUTED, line_dash="dash",
                              annotation_text="S₀ — giá tại thời điểm gốc")
                fig.add_vline(x=m["q_low"], line_color=ACCENT_DOWN,
            ...
            note("<b>Đặc tả mô hình mô phỏng.</b> Chuyển động Brown hình học giả định giá "
                 "tuân theo phương trình vi phân ngẫu nhiên dS = μS·dt + σS·dW. Áp dụng "
                 "<b>bổ đề Itô</b> cho hàm ln(S) thu được nghiệm dạng đóng "
                 "S<sub>t</sub> = S₀·exp[(μ − ½σ²)t + σW<sub>t</sub>], và đây chính là công "
                 "thức được mô phỏng. Số hạng hiệu chỉnh <b>−½σ²</b> phát sinh vì ln là hàm "
                 "lõm nên theo bất đẳng thức Jensen E[ln S] ≠ ln E[S]; bỏ số hạng này sẽ "
                 "làm kỳ vọng bị thổi lên đúng hệ số exp(σ²T/2), sai lệch tăng theo bình "
                 "phương độ biến động. Phương án <b>Bootstrap</b> thay bước lấy mẫu Z ~ N(0,1) "
                 "bằng việc rút mẫu có hoàn lại từ chính chuỗi lợi suất lịch sử, do đó giữ "
                 "nguyên đuôi dày và độ lệch của phân phối thực nghiệm nhưng vẫn giả định "
                 "các quan sát độc lập cùng phân phối — nghĩa là vẫn bỏ qua hiện tượng tụ "
                 "cụm biến động (volatility clustering) mà các mô hình họ GARCH xử lý. "
                 "Mô phỏng dùng seed cố định nên kết quả tái lập được; sai số Monte Carlo "
                 "giảm theo tốc độ 1/√N với N là số kịch bản.")
            ...
            if res is not None:
                ...
                with cA:
                    ...
                    fig = px.scatter(d, x="rm_ex", y="ri_ex", trendline="ols",
                                     opacity=0.55, trendline_color_override=ACCENT)
                    ...
                    st.plotly_chart(apply_theme(fig, 420, "Đường đặc trưng chứng khoán"),
                                    use_container_width=True)
                    note("<b>Đường đặc trưng chứng khoán (Security Characteristic Line).</b> "
                         "Đây là đồ thị phân tán của hồi quy Rᵢ − R_f = α + β(R_m − R_f) + ε, "
                         "với mỗi điểm là một phiên giao dịch. <b>Hệ số góc</b> của đường hồi "
                         "quy là beta — độ nhạy của cổ phiếu với rủi ro hệ thống; <b>hệ số "
                         "chặn</b> là alpha Jensen — phần lợi suất vượt trội không giải thích "
                         "được bằng rủi ro thị trường. Cả hai biến đều là <i>lợi suất vượt "
                         "trội</i> (đã trừ lãi suất phi rủi ro), đúng như dạng thức CAPM yêu "
                         "cầu, nên hệ số chặn đọc trên hình trùng với Alpha báo cáo phía trên. "
                         "Nếu vẽ trên lợi suất thô, hệ số chặn thu được sẽ là α + R_f(β − 1) "
                         "và không còn là alpha Jensen. <b>Độ phân tán</b> của các điểm quanh "
                         "đường thẳng phản ánh rủi ro phi hệ thống — thành phần mà đa dạng hóa "
                         "danh mục có thể triệt tiêu; phần này tương ứng với 1 − R².")
                with cB:
                    ...
                    fig.add_trace(go.Scatter(x=[res["beta"]], y=[res["actual_return"]],
                                             ...
                                             marker=dict(size=15, color=ACCENT, symbol="star")))
                    ...
                    note(f"<b>Đường thị trường chứng khoán (SML).</b> SML biểu diễn quan hệ "
                         f"tuyến tính E(Rᵢ) = R_f + βᵢ[E(R_m) − R_f] mà CAPM tiên đoán: ở "
                         f"trạng thái cân bằng, mọi tài sản phải nằm <i>trên</i> đường này, và "
                         f"beta là thước đo rủi ro duy nhất được thị trường trả giá. Điểm "
                         f"{ticker} hiện nằm <b>{pos}</b> đường SML — lợi suất thực hiện "
                         f"{res['actual_return']:+.2%} so với mức yêu cầu "
                         f"{res['required_return']:+.2%} ứng với beta {res['beta']:.2f}; chênh "
                         f"lệch theo phương thẳng đứng chính là alpha Jensen. Lưu ý về mặt "
                         f"phương pháp: sai lệch khỏi SML có thể do định giá sai, nhưng cũng có "
                         f"thể do mô hình thiếu nhân tố hoặc do danh mục đại diện thị trường "
                         f"không phải danh mục thị trường thật — <b>phê phán Roll (1977)</b> "
                         f"cho rằng CAPM về nguyên tắc không kiểm định được vì lý do sau cùng "
                         f"này. Vì vậy alpha nên được đọc kèm mức ý nghĩa thống kê, không phải "
                         f"như một kết luận chắc chắn về định giá sai.")
