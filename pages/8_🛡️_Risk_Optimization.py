...
def render_risk_page():
    ...
    c2.metric("Value at Risk (VaR 1 Ngày)", f"-{var_metrics['var_amount']:,.0f} VND", f"-{var_metrics['var_pct']*100:.2f}%")
    c3.metric("Conditional VaR (CVaR 1 Ngày)", f"-{var_metrics['cvar_amount']:,.0f} VND", f"-{var_metrics['cvar_pct']*100:.2f}%")
    ...
    fig.add_trace(go.Scatter(
        ...
        mode='markers',
        marker=dict(color=ef_results[2, :], colorscale='Viridis', showscale=True, size=5, title="Sharpe"),
