...
Ba yêu cầu về phương pháp mà module này bảo đảm:

  1. MỌI HỆ SỐ ĐI KÈM KIỂM ĐỊNH. Alpha và beta luôn được báo cáo cùng sai số chuẩn,
     t-statistic và p-value. Một alpha bằng 0,000123 với t = 0,3 không khác 0 về mặt
     thống kê, nên kết luận "cổ phiếu tạo ra alpha" từ riêng dấu của hệ số là không
     có căn cứ.
  2. ALPHA ĐƯỢC NĂM HÓA trước khi hiển thị, để con số đọc được cùng thang với lợi
     suất yêu cầu và so sánh được giữa các tần suất dữ liệu khác nhau.
  3. HÀM TRẢ VỀ CHÍNH BỘ DỮ LIỆU ĐÃ HỒI QUY (khóa 'data'). Hồi quy thực hiện trên
     lợi suất vượt trội, nên biểu đồ phải vẽ trên cùng bộ dữ liệu đó thì hệ số chặn
     đọc trên hình mới trùng với alpha báo cáo; vẽ trên lợi suất thô sẽ cho hệ số
     chặn bằng α + R_f(β − 1), là một đại lượng khác.
"""
...
def capm(stock_ret: pd.Series, market_ret: pd.Series) -> dict:
    ...
    Beta ước lượng được sau đó dùng để tính lợi suất yêu cầu:
        E(R_i) = R_f + β · [E(R_m) − R_f]
    Đây mới là nội dung kinh tế của mô hình: CAPM là một quan hệ định giá cân bằng,
    beta chỉ là tham số trung gian. Chênh lệch giữa lợi suất thực hiện và lợi suất
    yêu cầu là alpha Jensen.
    """
