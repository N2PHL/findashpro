...
Sáu quy ước bắt buộc của module, mỗi quy ước tương ứng một nguồn sai lệch đã biết
trong mô phỏng giá tài sản:

  (a) Drift là tham số khai báo được (lịch sử / trung tính rủi ro / bằng 0), không
      mặc định bằng 0 — nếu ép bằng 0 thì E[S_T] luôn xấp xỉ giá hiện tại với mọi
      mã và mọi khung dự phóng, tức mô hình không còn nội dung kinh tế.
  (b) Số hạng hiệu chỉnh −½σ² trong số mũ là bắt buộc, theo bổ đề Itô áp dụng cho
      ln(S). Thiếu nó, kỳ vọng bị lệch lên đúng hệ số exp(σ²T/2).
  (c) Tích lũy theo exp(Σ log return), không dùng cumprod(1 + simple return) — trộn
      hai quy ước lợi suất trong cùng một mô hình log-normal là không nhất quán.
  (d) Độ lệch chuẩn tính với ddof = 1 (ước lượng không chệch cho mẫu), không phải
      công thức tổng thể.
  (e) Seed cố định để mọi kết quả mô phỏng tái lập được — điều kiện tối thiểu để
      một kết quả định lượng có thể kiểm chứng.
  (f) Đường mô phỏng bắt đầu đúng tại S₀, để dải phân vị nối liền với giá quan sát
      cuối cùng thay vì nhảy bậc ngay bước đầu tiên.
"""
...
def run_monte_carlo(
    ...
    drift_mode:
      ...
      zero         — drift bằng 0, tức E[S_T] = S₀. Lưu ý đây là kỳ vọng của giá,
                     đạt được nhờ số hạng hiệu chỉnh Itô −½σ² trong số mũ; đặt kỳ
                     vọng của simple return bằng 0 là một điều kiện khác.
    ...
    if drift_mode == "historical":
        ...
        # E[S_T] = S₀·exp(μT). Muốn kỳ vọng của GIÁ đứng yên thì μ = 0; số hạng −½σ²
        # trong bước mô phỏng chính là phần bù lại độ lồi của hàm mũ (bất đẳng thức
        # Jensen). Nếu thiếu số hạng đó, μ = 0 vẫn cho kỳ vọng tăng theo exp(σ²T/2).
        mu = 0.0
    else:
...
def mc_risk_metrics(sim: pd.DataFrame, confidence: float = 0.95) -> dict:
    ...
    VaR là một khoản lỗ, không phải một mức giá:
        VaR_95% = S₀ − Q_5%(S_T)
    Phân vị của giá cuối kỳ, Q_5%(S_T), chỉ là một thành phần của công thức; báo cáo
    thẳng phân vị giá dưới nhãn "VaR" là sai định nghĩa. CVaR lấy kỳ vọng có điều
    kiện trên toàn bộ vùng đuôi thay vì một điểm phân vị duy nhất.
    """
