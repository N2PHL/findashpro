# ui/charts.py
"""
Biểu đồ Plotly dùng chung cho toàn ứng dụng.

Quy ước trình bày:
  1. Giá và khối lượng nằm trên hai subplot chia sẻ trục thời gian, thay vì dùng
     trục y thứ cấp. Hai đại lượng này khác đơn vị và khác bậc độ lớn, nên ép chung
     một trục sẽ buộc phải co giãn thủ công và làm mất khả năng đọc giá trị thật.
  2. Cột khối lượng tô màu theo chiều biến động của phiên, cho phép đọc quan hệ
     giữa thanh khoản và hướng giá ngay trên cùng một hình.
  3. Một template thống nhất (utils.config.PLOTLY_TEMPLATE) cho mọi biểu đồ, để nền
     và bảng màu không đổi khi chuyển trang.

Bảng màu và font đều lấy từ utils.config, không khai báo lại tại chỗ.
"""
